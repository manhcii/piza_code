                            ...
                            <div class="tab-pane active" id="tab_1">
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label>
                                                @lang('Select Paramater')
                                                <small class="text-red">*</small>
                                            </label>
                                            <select name="json_params[paramater_id]" class="form-control select2"
                                                style="width: 100%" required>
                                                <option value="">@lang('Please select')</option>
                                                @foreach ($parameter as $item)
                                                    @if ($item->parent_id == 0 || $item->parent_id == null)
                                                        <option value="{{ $item->id }}"
                                                            {{ isset($detail->json_params->paramater_id) && $detail->json_params->paramater_id == $item->id ? 'selected' : '' }}>
                                                            {{ $item->name }} &nbsp;({{ $item->is_type }})
                                                        </option>
                                                    @endif
                                                @endforeach
                                            </select>
